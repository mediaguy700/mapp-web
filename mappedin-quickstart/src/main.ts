...
/** Load events from GET /events, derive children (unique by name+mac per reader) with reader coords */
...
async function loadReaders(mapView: Awaited<ReturnType<typeof show3dMap>>) {
  try {
    ...
    // Show readers and children in panel (always visible)
    ...
    // Add to outdoor map (GeoJSON layers)
    const addToOutdoor = () => {
    ...
    // Add Mappedin markers for readers (blue) and children (green)
    ...
    // Pan camera to show readers and children
...
function setupReaderDialog(children: Child[]) {
  ...
  // Use coordinate hit-test (like panel buttons) so clicks work when map intercepts
...
async function init() {
  const mapData = await getMapData(mappedinOptions);
  ...
  await loadReaders(mapView);
  // Ensure outdoor map is visible (readers are drawn on it)
